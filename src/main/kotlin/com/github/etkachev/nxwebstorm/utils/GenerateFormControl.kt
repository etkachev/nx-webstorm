...
class GenerateFormControl(private val required: JsonArray?) {
    ...
    fun getFormControl(prop: JsonObject, name: String): FormCombo? {
        ...
        val result = when (type) {
            "boolean" -> FormCombo(getBoolControl(description, default), FormControlType.BOOL, name, description, null, requiredFields)
    ...
    private fun getFormComboOfString(name: String, description: String?, enums: JsonArray?, default: String?): FormCombo {
        if (enums == null) {
        ...
        return FormCombo(getSelectDropdown(stringOptions, default), FormControlType.LIST, name, description, stringOptions, requiredFields)
    }
